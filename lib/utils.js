...
export async function loadGetRenderProps (Component, initialProps, ctx, ssr) {
  if (!Component.getRenderProps) return initialProps

  const renderProps = await Component.getRenderProps(initialProps, ctx, ssr)

  if (typeof renderProps === 'undefined') return initialProps

  if (!renderProps) {
    const compName = getDisplayName(Component)
    const message = `"${compName}.getRenderProps()" should resolve to an object. But found "${renderProps}" instead.`
    throw new Error(message)
  }
